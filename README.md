...
截至 2026-06-05，项目处于博士课题主线的早期工程化阶段。当前重点不是直接生成完整 CAD 模型，而是先把图纸理解输入质量、数据流处理、视图裁剪和小模型筛选 benchmark 做成可复现的工程基础。
...
- 初步实现 `clean-layout` 页面级清理流程，可识别并移除外边框、标题栏、左侧孔表、右上角版本表等非视图区表格，同时将被移除表格分类保存为 crop，供后续语义抽取使用；
- 已在 `DataFlow/LayoutSamples/` 和 `DataFlow/03.LayoutAnalysis/`、`DataFlow/04.CleanPNG/` 上进行若干样例验证，当前清理策略以“保留视图及其相关标注、去除表格和边框”为目标。
...
```text
第二阶段：图纸理解和中间表示抽取
当前子阶段：layout cleaning、view crop 准备与小模型 benchmark 联合验证
```
...
- 建立自动 `05.ViewDetection` 与 `06.SingleViews` 视图裁剪流程；
...
- 为 DrawingIR、尺寸-几何绑定和约束图构建提供可靠输入。
...
1. 批量验证 `clean-layout`，对样例标记“清理成功、漏删表格、误删标注、误删视图”等质量标签；
2. 实现 `05.ViewDetection` 的第一版规则化视图区检测，输出视图 bbox、置信度和来源规则；
3. 实现 `06.SingleViews` 的正式产物规范，每个视图至少保留 raw view、clean view 和必要的 annotation metadata；
4. 将 `DataFlow/06.SingleViews/testView2CAD/` 中外部切好的 crops 作为临时样例，先打通 `single-view crops -> VLM/规则抽取 -> 最小 DrawingIR -> CadQuery 代码草稿` 的后半段原型；
5. 对样本进行最小人工标注，包括视图数量、视图类型、主视图、尺寸文本、特征数量和难度等级；
6. 跑通 `view_count`、`view_classification`、`dimension_ocr`、`feature_count`、`json_stability` 五个小模型筛选任务，并比较 full page、clean page、single-view crop 三种输入；
7. 基于评测结果确定主 VLM、OCR 工具、layout/view crop 工具组合和后续 DrawingIR pipeline。
...
| 阶段 | 目标 | 主要产物 | 状态 |
| --- | --- | --- | --- |
| 阶段 1：课题收敛与数据准备 | 明确博士主线、样本范围和数据流 | README、DataFlow、初始 PDF/STEP 数据 | 已启动并持续补充 |
| 阶段 2：图纸理解与模型筛选 | 评估小参数 VLM/OCR 模型的图纸理解能力，建立 layout/view crop 数据流 | model-screening benchmark、layout cleaning、view crop、评测报告 | 当前阶段 |
| 阶段 3：DrawingIR 构建 | 从 page/view crops 抽取视图、尺寸、标注和特征候选 | DrawingIR JSON、标注规范 | 准备进入最小原型 |
