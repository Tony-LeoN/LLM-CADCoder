...
截至 2026-06-03，项目处于博士课题主线的早期工程化阶段。当前重点不是直接生成完整 CAD 模型，而是先建立图纸理解、数据流处理和小模型筛选 benchmark 的基础框架。
...
```text
第二阶段：图纸理解和中间表示抽取
当前子阶段：小模型筛选 benchmark 与 PDF 图纸数据流规范化
```
...
1. 完成 `DataFlow/01.RawPDFWithSTEP/` 中 PDF 与 STEP 的样本配对检查；
2. 将 `PDF2PNG.py` 的逻辑迁移为正式的 `pdf_render` stage，统一输出 PNG 和 `render_meta.json`；
3. 选取 20-50 个板类/法兰类/轴套类样本，建立 `benchmark_small`；
4. 对样本进行最小人工标注，包括视图数量、视图类型、主视图、尺寸文本、特征数量和难度等级；
5. 在服务器端配置模型环境，优先接入 `Qwen3-VL-2B-Instruct`、`Qwen3-VL-2B-Thinking`、`InternVL3.5-2B` 和 `PaddleOCR-VL`；
6. 跑通 `view_count`、`view_classification`、`dimension_ocr`、`feature_count`、`json_stability` 五个小模型筛选任务；
7. 基于评测结果确定主 VLM、OCR 工具和后续图纸理解 pipeline。
...
| 阶段 | 目标 | 主要产物 | 状态 |
| --- | --- | --- | --- |
| 阶段 1：课题收敛与数据准备 | 明确博士主线、样本范围和数据流 | README、DataFlow、初始 PDF/STEP 数据 | 已启动 |
| 阶段 2：图纸理解与模型筛选 | 评估小参数 VLM/OCR 模型的图纸理解能力 | model-screening benchmark、评测报告 | 当前阶段 |
| 阶段 3：DrawingIR 构建 | 从 PDF/PNG 抽取视图、尺寸、标注和特征候选 | DrawingIR JSON、标注规范 | 待完成 |
| 阶段 4：尺寸-几何绑定与约束图 | 建立尺寸标注、几何实体和多视图关系 | ConstraintGraph、绑定算法、F1 指标 | 待完成 |
| 阶段 5：CadQuery 代码生成 | 将结构化图纸知识转为参数化建模脚本 | CadQuery 代码、STEP 模型 | 待完成 |
