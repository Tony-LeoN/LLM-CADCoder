...
```text
第二阶段：图纸理解和中间表示抽取
当前子阶段：01-06 数据流验收、视图分类准备与 DrawingIR 最小原型设计
```
...
5. 建立 `07.ViewClassification` 最小流程，优先判断主视图、侧视图、俯视图、轴测图和剖视/局部视图；
6. 设计 `08.Multi-viewFeatureExtraction` 与 `10.StructuredCADRepresentation` 的最小 schema，先覆盖板类件中的孔、槽、倒角、圆角、阵列和厚度/拉伸方向；
...
8. 基于评测结果确定主 VLM、OCR 工具、layout/view crop 工具组合和后续 DrawingIR pipeline。
...
| 阶段 2：图纸理解与模型筛选 | 评估小参数 VLM/OCR 模型的图纸理解能力，建立 layout/view crop 数据流 | model-screening benchmark、layout cleaning、view crop、评测报告 | `01 -> 06` 已跑通，当前验收中 |
